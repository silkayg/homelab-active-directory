...
## Status
✅ Abgeschlossen — Lab vollständig funktionsfähig, Domain-Join und
GPO-Anwendung verifiziert.
...
- **Client (`client01`):** Windows 11 Enterprise (Eval),
  4 GB RAM, 2 vCPU, 50 GB HDD, EFI + TPM 2.0
- **Domäne:** `lab.local` (NetBIOS: `LAB`)

## Sitzung 1: Server-Installation und DC-Promotion

### Vorbereitung
- Windows Server 2022 Eval-ISO vom Microsoft Evaluation Center heruntergeladen
...
- Verifiziert in Sitzung 2/3 mit Client-VM
...
- Lehrreicher Effekt: Im echten Lab muss die Trennung zwischen
  Client-Netz und externem Internet sauber sein, sonst gehen
  DNS-Anfragen am DC vorbei.
...
- Active-Directory-Aufbau verstanden: Forest, Domain, OU, User,
  Gruppe, GPO
- Praktischer Unterschied zwischen Windows-Editionen Pro/Enterprise
  (domänenfähig) vs. Home (nicht domänenfähig)
- DNS ist im AD-Kontext kritisch — der DC muss DNS-Server für die
  Clients sein, sonst funktioniert keine Domänen-Auflösung
...
- `gpupdate /force` als manueller Refresh, statt auf Standard-Zyklus
  (alle 90 Min ± 30 Min Zufallsverzögerung) zu warten
- GPO-Wirkung mit `gpresult /r` auf dem Client verifizierbar
